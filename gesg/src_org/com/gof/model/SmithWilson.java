...
//		sw.getSmithWilsonResultMap().entrySet().stream().filter(s-> Double.parseDouble(s.getKey().substring(1, 5)) >= 1190).forEach(s->log.info("{}, {}, {}", s.getKey(), s.getValue().getSpotDisc(), s.getValue().getFwdDisc()));
	...
	//TODO; 
	public SmithWilson(LocalDate baseDate, List<IrCurveSpot> irCurveHisList,                boolean isRealNumber, double ltfr, int ltfrT, int prjYear) {
		this(baseDate, irCurveHisList, CMPD_MTD_DISC, isRealNumber, ltfr, ltfrT, prjYear, 1, 20, 1);		
	}	

	public SmithWilson(LocalDate baseDate, List<IrCurveSpot> irCurveHisList, char cmpdType, boolean isRealNumber, double ltfr, int ltfrT, int prjYear, int dayCountBasis) {
		this(baseDate, irCurveHisList, cmpdType     , isRealNumber, ltfr, ltfrT, prjYear, 1, 20, dayCountBasis);		
	}
	
	public SmithWilson(LocalDate baseDate, List<IrCurveSpot> irCurveHisList, char cmpdType, boolean isRealNumber, double ltfr, int ltfrT, int prjYear, int prjInterval, int alphaItrNum, int dayCountBasis) {				
		super();		
