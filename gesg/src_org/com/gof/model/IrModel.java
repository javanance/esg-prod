...
public abstract class IrModel {
	...
	protected static final int                 DECIMAL_DIGIT        = 17;
...
//		this.tenor = irCurveHis.stream().map(s -> Double.parseDouble(s.getMatCd().substring(1, 5)) / 12.0).mapToDouble(Double::doubleValue).toArray();
	...
	public static TreeMap<String, Double> irSpotDiscToFwdM1Map(TreeMap<String, Double> spotRateMap) {
		...
		double[] prjTenor = spotRateMap.entrySet().stream().map(s -> Double.valueOf(1.0 * Integer.valueOf(s.getKey().substring(1)) / MONTH_IN_YEAR)).mapToDouble(Double::doubleValue).toArray();
	...
	public static TreeMap<String, Double> irSpotDiscToFwdMap(String bssd, TreeMap<String, Double> spotRateMap, String fwdMatCd) {		
		...
		for(int i=0; i<matCd.length; i++) tsMap.put(prjTenor[i], spotDisc[i]);
	    ...
	    double[] swSpotCont = swRsltList.stream().map(s -> s.getSpotCont()).mapToDouble(Double::doubleValue).toArray();
	    double[] swSpotDisc = swRsltList.stream().map(s -> s.getSpotDisc()).mapToDouble(Double::doubleValue).toArray();
...
//		log.info("{}, {}, {}", upCnt, dnCnt, runCnt);
		...
		double mu = (2.0 * vec.length - 1) / 3;
		double sd = Math.sqrt((16.0 * vec.length - 29) / 90);
		double zStar = (runCnt - mu) / sd;
