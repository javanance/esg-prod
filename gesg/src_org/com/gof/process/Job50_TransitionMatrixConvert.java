 ...
 */
@Slf4j
public class Job50_TransitionMatrixConvert {
	public static List<RcCorpPd> createCorpPdFromUd(String bssd) {
		List<RcCorpPd> rstList = new ArrayList<RcCorpPd>();
		RcCorpPd temp;

		List<_TransitionMatrixUd> tmUd = _TransitionMatrixDao.getDefaultRateUd(bssd);
//		rst.forEach(s -> log.info("zzz  : {}", s.getFromGrade()));

//		Map<String, Double> volMap = getVolMap(bssd);

		for (_TransitionMatrixUd aa : tmUd) {
			temp = new RcCorpPd();

			temp.setBaseYymm(bssd);
			temp.setCrdEvalAgncyCd("01");
			temp.setCrdGrdCd(ECrdGrd.getECrdGrd(aa.getFromGrade()).getLegacyCode());
			temp.setCumPd(aa.getTmRate());
//			temp.setVol(volMap.getOrDefault(temp.getCrdGrdCd(), 0.0));
		...
		log.info("Job51( Corporate PD Calculation) creates  {} results.  They are inserted into EAS_CORP_CRD_GRD_PD Table", rstList.size());
	...
	public static List<_CorpCumPd> createCorpCumPdFromUd(String bssd) {
		List<_CorpCumPd> rstList = new ArrayList<_CorpCumPd>();
		_CorpCumPd temp;

		List<_TransitionMatrixUd> rst = _TransitionMatrixDao.getTMUd(bssd);
		...
		int n = 0;
		int m = 0;
		for (_TransitionMatrixUd aa : rst) {
//			logger.info("Corp Cum PD : {},{},{}", aa.getFromGradeEnum().getAlias(), aa.getToGradeEnum().getAlias(),aa.getTmRate());
		...
		log.info("Job51( Corporate Cumulative PD Calculation) creates  {} results.  They are inserted into EAS_CORP_CRD_GRD_CUM_PD Table", rstList.size());
